...
format:
    html:
        theme: "../biplaRquarto/inst/extdata/_extensions/biplaR-html/style/bipla.scss"
        toc: true
        toc-location: left
        toc-title: "Inhalt"
        ...
        css: "../biplaRquarto/inst/extdata/_extensions/biplaR-html/style/logo.css"
    pdf: 
        theme: "../biplaRquarto/inst/extdata/_extensions/biplaR-html/style/bipla.scss"
        toc: true
        toc-location: left
...
d <- bind_rows(select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
        ...
        group_by(year) %>% 
        summarise_all(mean, na.rm=T) %>%
        pivot_longer(cols = zufrieden, names_to = "indicator", values_to = "mean") %>% 
        mutate(profil = "Total"),
    select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
    filter(!is.na(profil) & !str_detect(profil, "Phil|Lehr")) %>% 
    group_by(year, profil) %>% 
    summarise_all(mean, na.rm=T) %>%
    pivot_longer(cols = zufrieden, names_to = "indicator", values_to = "mean")) %>%
    mutate(indicator = case_when(indicator == "vorb_fach" ~ "Fachliche Vorbereitung"),
        profil = fct_relevel(profil, "Total"),
        text = paste0(profil, ": ", round(mean, 2)), 
        profil = factor(profil, levels = c("Altsprachlich", "Mathematisch-Naturwissenschaftlich", "Neusprachlich", "Wirtschaftlich-Rechtlich","Musisch", "Total")),
        indicator = factor (indicator, levels = c("Zufriedenheit mit der Ausbildung", "Fachliche Vorbereitung", "Persönlichkeitsbildende Vorbereitung", "Vorbereitung auf selbständiges Arbeiten", "Total")))
...
## Einschätzung der fachlichen Vorbereitung

### nach Profil

```{r, plot fachliche Vorbereitung}
p <- get_pool()
...
d <- bind_rows(select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
        ...
        group_by(year) %>% 
        summarise_all(mean, na.rm=T) %>%
        pivot_longer(cols = vorb_fach, names_to = "indicator", values_to = "mean") %>% 
        mutate(profil = "Total"),
    select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
    filter(!is.na(profil) & !str_detect(profil, "Phil|Lehr")) %>% 
    group_by(year, profil) %>% 
...
## Einschätzung der persönlichkeitsbildenden Vorbereitung

### nach Profil

```{r, plot persönlichkeitsbildende Vorbereitung}
p <- get_pool()
...
d <- bind_rows(select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
        ...
        group_by(year) %>% 
        summarise_all(mean, na.rm=T) %>%
        pivot_longer(cols = vorb_pers, names_to = "indicator", values_to = "mean") %>% 
        mutate(profil = "Total"),
    select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
    filter(!is.na(profil) & !str_detect(profil, "Phil|Lehr")) %>% 
    group_by(year, profil) %>% 
...
## Einschätzung der Vorbereitung auf selbständiges Arbeiten

### nach Profil

```{r, plot Vorbereitung auf selbständiges Arbeiten}
p <- get_pool()
...
d <- bind_rows(select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
        ...
        group_by(year) %>% 
        summarise_all(mean, na.rm=T) %>%
        pivot_longer(cols = vorb_selbst, names_to = "indicator", values_to = "mean") %>% 
        mutate(profil = "Total"),
    select(p, year, profil, zufrieden, vorb_fach, vorb_pers, vorb_selbst) %>% 
    filter(!is.na(profil) & !str_detect(profil, "Phil|Lehr")) %>% 
    group_by(year, profil) %>% 
...
d <- d %>% 
  filter(year %in% c(2015, 2018, 2021, 2024))
...
plot <- ggplot(d, aes(x=year, y=mean, group=profil, colour=profil,
    ...
    biplaR::scale_colour_zhweb() +
    guides(colour = guide_legend(ncol = 2)) +
    scale_x_continuous(breaks = seq(2015, 2024, 3), minor_breaks = NULL) +
    scale_y_continuous(limits = c(1,6), breaks = seq(1, 6, 1))+
    biplaR::getTheme(c("no_axis_title", "no_legend_title")) +
    theme(panel.grid.major.y = element_line(color = "darkgrey", linewidth =  0.05),      # Optional: keep minor vertical lines
    panel.grid.major.x = element_blank(),                  # Remove horizontal grid lines
...
## Abbildung: Einschätzung der überfachlichen Kompetenzen, 2015-2024
